...
extension MainService: MainServiceProtocol {
    ...
    func retrieveMarvelCharacters(offset: Int64, completion: @escaping GenericResult<CharacterDataResponse>) {
        ...
        network.execute(endpoint: MainEndpoints.getCharacters(offset: offset,
                                                              timestamp: timestamp,
                                                              apikey: Keys.pubKey.rawValue,
                                                              hash: Crypto.getAuthorization(intTs: timestamp))) {( success: Bool,
                                                                response: ResponseData<CharacterDataResponse>?,
                                                                                                                error: ErrorData<ErrorSpringCodable>?) in
